...
class Movement:
	...
	rospy.loginfo("Inside move_to_goal_point()")
        ...
        # IS NOT UPDATING TO THE NEW ANGLE SEEN, SO IT IS STUCK AT 0.78
            ...
            elif abs(angle_to_goal - self.theta) > 0.1: # not angled at the point we are trying to reach
                    # Spin clockwise around a circle
                    # rospy.loginfo("Current angle: " + str(abs(angle_to_goal - theta)))
                    self.move.linear.x = 0.0
